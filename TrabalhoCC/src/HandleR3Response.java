            ...
            try {
                switch (r3Package.tipo) {
                    case "GET":
                        ...
                        int tamanho = 40;
                        int size = (int) Math.ceil(response.length() / (tamanho * 1.0)); // Tamanho maximo da string de
                                                                                         // pacote são 40 bytes
                        String strings[] = new String[size];
                        for (int i = 0; i < size; i++)
                            strings[i] = response.substring(i * tamanho,
                                    Math.min((i + 1) * tamanho - 1, response.length() - 1));
                        synchronized (totais) {
                        ...
                        out.flush();
                        s.close();
                        response = "BEGIN" + " " + "MSG" + " " + "0"+ " " + size + " " + r3Package.clientAddress + " "
